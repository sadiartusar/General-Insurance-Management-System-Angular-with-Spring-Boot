...
@Component({
  selector: 'app-updatecarpolicy',
  standalone: false,
  templateUrl: './updatecarpolicy.html',
  styleUrl: './updatecarpolicy.css'
})
export class Updatecarpolicy implements OnInit{

   policy: CarModel = new CarModel();
    carPolicyId !: number;
    ...
    ngOnInit(): void {
       ...
       this.carPolicyId = this.route.snapshot.params['id'];
      ...
      // Fetch the Car Policy data by ID
      this.carPolicyService.getByCarPolicyId(this.carPolicyId)
        .subscribe({
          next: res => {
            this.policy = res;
    ...
    updateCarPolicy() {
      ...
      this.policy.id=this.carPolicyId
      this.carPolicyService.updateCarPolicy(this.carPolicyId, this.formValue.value)
        .subscribe({
          next: res => {
            console.log(res);
            this.formValue.reset();
            this.router.navigate(['/viewcarpolicy']); // Navigate back to the policy list after successful update
          },
          error: error => {
            console.log(error);
          }
